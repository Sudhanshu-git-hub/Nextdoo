...
## PC6 acceptance boundary — Insights, Summary and Reports

**IMPLEMENTED:** `/insights` and `/insights/reports` combine Tasks, Goals/Milestones, Tracker, Calendar, Knowledge/Data, recorded focus minutes, recent activity and upcoming deadlines. Daily, weekly, monthly, quarterly, yearly and custom reports use workspace-local periods; Tracker dates retain each tracker's clock. Current goal progress uses Goal Center's existing calculation, and Tracker reports use saved scores and existing denominator rules. No second execution or scoring engine is introduced. Existing `/analytics` remains available.

Activity counts cover selected dates. Current overdue work, active goals, measured goal progress, remaining milestones and database inventory are explicitly current-state snapshots. Completion rate uses the existing task due-date cohort and exclusion rules; completion activity uses retained completion timestamps. Reopening, deletion and corrections can change later reports. Calendar workload uses visible sources, separate all-day counts and clipped scheduled minutes; overlapping events add individually. Scheduled minutes do not imply completed work. Recorded timer minutes belong to their session start date.

**IMPLEMENTED:** Accessible task bars, numerical tables, goal progress bars, tracker daily/weekly/monthly trends and responsive cards. Adjacent previous periods contain the same number of local dates; comparison requires a completed period and the user's comparison preference. Tracker comparisons also require equal eligible dates. Recording streaks are bounded by the selected range, with an ending streak only if its final eligible date was recorded. Numeric scores, streaks and comparisons obey Wellbeing controls, including downloads; comparisons remain hidden by default.

**IMPLEMENTED:** Authenticated private/no-store read APIs and CSV/JSON report snapshots, CSV formula escaping, browser print reports, workspace ownership and source-purge semantics. Server-side aggregation and 1–366-date limits avoid full history downloads. Reports show up to 50 goal cards (full aggregate counts), 20 projects, 20 recent activities and 20 upcoming deadlines. More than 200 non-archived trackers or 50,000 selected tracker records produces an explicit range/size error instead of partial totals. Read-only repeatable-read transactions provide one consistent source snapshot. PC6 adds no persisted report data, migration, cache, provider credentials or background worker.

**PARTIAL:** Knowledge insights cover database inventory and note/record creations. Goal trends show actual completion dates; historical goal-progress percentages are not stored and cannot be compared. Tracker results are stored observations, with existing asynchronous ingestion freshness rather than on-demand rescoring. Calendar actual-work comparisons are limited to separately labeled task completion and recorded focus evidence.
