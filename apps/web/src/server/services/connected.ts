...
export async function connectedToday(workspaceId:string,userId:string,now=new Date()) {
  ...
  return withTransaction(async db=>{
    ...
    const calendar=await db.execute<ConnectedItem>(sql`select e.id,'calendar' type,coalesce(e.title,'Calendar event') title,'/calendar' href,to_char(e.starts_at at time zone ${workspace.timeZone},'HH24:MI')||case when e.is_all_day then ' · all day' else '' end detail from calendar_events e join calendar_connections c on c.id=e.connection_id where e.workspace_id=${workspaceId} and c.user_id=${userId} and c.status='ACTIVE' and e.starts_at<=${bounds.end.toISOString()}::timestamptz and e.ends_at>${bounds.start.toISOString()}::timestamptz order by e.starts_at,e.id limit 7`);
